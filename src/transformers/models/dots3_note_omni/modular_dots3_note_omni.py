...
"""PyTorch Dots3-Note Omni model for Hugging Face Transformers."""
...
class Dots3NoteOmniTextAttention(nn.Module):
    def __init__(self, config: Dots3NoteOmniConfig, layer_idx: int, is_sliding: bool = False):
        ...
        if self.q_lora_rank is not None:
            ...
            self.q_a_layernorm = (
                _get_norm(self.q_lora_rank, config.rms_norm_eps, norm_type) if config.qk_layernorm else nn.Identity()
            )
        ...
        self.kv_a_layernorm = (
            _get_norm(self.kv_lora_rank, config.rms_norm_eps, norm_type) if config.qk_layernorm else nn.Identity()
        )
